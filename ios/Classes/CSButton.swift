...
class CSButton: UIButton {
    ...
    var imagePositionMode: CSButtonImagePositionStyle = .right
   ...
   private var imageSize: CGSize = CGSize.zero
    ...
    // MARK: ---------------private-------------------------
    init(frame: CGRect, imagePositionMode: CSButtonImagePositionStyle = .right) {
    ...
    override func layoutSubviews() {
        ...
        imageSize = imageView?.image?.size ?? CGSize.zero
    ...
    private func adjustTitleSize() -> CGSize {
        ...
        let titleSize = ((titleLabel?.text ?? "") as NSString).boundingRect(with: CGSize(width: CGFloat(MAXFLOAT), height: CGFloat(MAXFLOAT)), options: NSStringDrawingOptions.usesLineFragmentOrigin, attributes: [NSAttributedString.Key.font : titleLabel!.font], context: nil).size
...
extension CSButton {
    var additionInfo: Int {
        set {
            objc_setAssociatedObject(self, &btnAdditionInfoKey, newValue, objc_AssociationPolicy.OBJC_ASSOCIATION_ASSIGN)
        }
        get {
            if let rs = objc_getAssociatedObject(self, &btnAdditionInfoKey) as? Int {
                return rs
            }
            return 0
        }
